...
# popularity + year??
# energy + loudness —> start
# danceability + valence
# energy + valence
# not acousticness (complete opposite)
# fun: low dance, high acousticness

# 1, rename duplicate removing function
# 2. rename variables used throughout FindSongs function to be better understandable/make more sense
# 3. write better comments
# 4. use constants
# 5. simplify reused code
# 6. add colors to terminal

# 1. change iterating variables
# 2. debug popularity shortcut code and determine how many popular songs to add
...
def find_songs(analysis_type, album_uri):
    ...
    for c in tracks["items"]:
        if c["duration_ms"] >= 60000 and "intro" not in c["name"].lower() and "outro" not in c["name"].lower() and "skit" not in c["name"].lower() and "speaks" not in c["name"].lower() and "instrumental" not in c["name"].lower():
